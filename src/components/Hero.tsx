...
export const Hero = ({ accentColor = '#94C11F' }: HeroProps) => {
  return (
    <section id="hero" className="min-h-screen relative overflow-hidden bg-brand-dark flex flex-col justify-center">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-32 pb-16 relative z-10 w-full flex flex-col md:flex-row items-center gap-16">
        <div className="max-w-2xl flex-1 text-center md:text-left">
          <div 
            className="inline-flex items-center gap-3 bg-accent/10 border border-accent/20 px-5 py-2 mb-8 animate-fade-in"
          ...
          <h1 className="font-display text-5xl md:text-7xl font-extrabold text-white leading-[1.1] mb-6 animate-slide-up">
            Innovación{" "}
          ...
          <p className="text-white/60 text-lg md:text-xl leading-relaxed mb-10 max-w-lg animate-slide-up [animation-delay:200ms]">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-4 animate-slide-up [animation-delay:400ms]">
            <button 
              className="bg-accent hover:bg-accent-dark text-brand-dark px-10 py-4 font-bold uppercase tracking-wider transition-all"
              style={{ clipPath: 'polygon(10px 0%, 100% 0%, calc(100% - 10px) 100%, 0% 100%)' }}
            >
              Ver Catálogo
            </button>
            <button 
              className="bg-transparent border border-white/30 text-white hover:bg-white/5 px-10 py-4 font-medium transition-all"
              style={{ clipPath: 'polygon(10px 0%, 100% 0%, calc(100% - 10px) 100%, 0% 100%)' }}
            >
              Contáctanos
            </button>
        ...
        <div className="flex-1 flex justify-center animate-scale-in">
          <div 
            className="w-[280px] md:w-[400px] aspect-[1/1.15] relative flex items-center justify-center bg-gradient-to-br from-accent/20 to-accent/5"
          ...
          >
             ...
             ></div>
             <img src="/assets/logo-perlad.png" alt="Perlad" className="w-2/3 brightness-0 invert drop-shadow-2xl z-10" />
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-wrap justify-center md:justify-start gap-12 md:gap-20">
          {[
            ...
            <div key={i} className="flex items-center gap-4">
              <Diamond size={40} bg={`${accentColor}22`}>
              ...
              <div>
                ...
                <div className="text-white/40 text-xs uppercase tracking-widest">{d.label}</div>
              </div>
            </div>
          ))}
        </div>
